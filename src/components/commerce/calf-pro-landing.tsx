...
const protocolSteps = [
  ["Relax", "Fasten the device comfortably and start with the lower intensity routine for daily relaxation."],
  ["Reset", "Use rhythmic air compression and heated surface support to make the session feel clear and structured."],
  ["Return", "Let the 15-minute auto-off close the session, then remove the device and get back to movement."]
];
...
export function CalfProLanding({ product, relatedProducts }: CalfProLandingProps) {
  ...
  const sceneImage = mainImages.find((image) => image.alt.toLowerCase().includes("lifestyle")) ?? mainImages[1] ?? mainImages[0];
  const mechanismImage = detailImages.find((image) => image.src.includes("inflation")) ?? detailImages[0] ?? mainImages[0];
  const detailBanner = detailImages.find((image) => image.src.includes("banner")) ?? detailImages[detailImages.length - 1] ?? mainImages[0];
      ...
      <section className="bg-monauro-teal py-16 text-white lg:py-20">
        <div className="grid page-shell gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          <div>
            ...
            <h2 className="mt-5 max-w-xl text-4xl font-semibold leading-tight md:text-6xl">Relief and recovery without a complicated setup.</h2>
      ...
      <section className="bg-white py-16">
        <div className="page-shell">
          <div className="grid gap-5 md:grid-cols-3">
            {mainImages.slice(1, 4).map((image, index) => (
              <article className="group relative min-h-[420px] overflow-hidden rounded-monauro bg-[#f7f7f4]" key={image.src}>
                <Image className="object-cover transition duration-500 group-hover:scale-[1.03]" src={image.src} alt={image.alt} fill sizes="(min-width: 1024px) 30vw, 100vw" />
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/72 to-transparent p-6 text-white">
                  ...
                  <h3 className="mt-3 text-2xl font-semibold">{protocolSteps[index]?.[0]}</h3>
                  <p className="mt-3 text-sm leading-6 text-white/78">{protocolSteps[index]?.[1]}</p>
                </div>
              </article>
            ))}
          </div>
        ...
        <div className="grid page-shell gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <div>
            <SectionHeader
              ...
              description="The manual describes three groups of six massage heads that inflate and deflate rhythmically. This section should later be replaced by an approved product animation or GIF."
            />
