...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link className="rounded-monauro border border-black/10 bg-white p-5 transition hover:-translate-y-1 hover:border-black/25" href={`/products/${product.slug}`}>
      <div className="relative flex aspect-square items-center justify-center overflow-hidden rounded-monauro border border-black/10 bg-[#f3f3ef] text-center text-sm text-neutral-500">
        {cardImage ? (
